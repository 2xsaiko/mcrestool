...
macro_rules! do_impl_deserialize_wrap {
    (struct $target:ident $($el:ident)* $((default $default:expr))?) => {
        impl<'de> $crate::BinDeserialize<'de> for $target {
            fn deserialize<D: $crate::BinDeserializer<'de>>(mut deserializer: D) -> $crate::Result<Self> {
                $(let $el = $crate::BinDeserialize::deserialize(&mut deserializer)?;)*
            ...
            fn deserialize_in_place<D: $crate::BinDeserializer<'de>>(
                ...
                $($crate::BinDeserialize::deserialize_in_place(&mut self.$el, &mut deserializer)?;)*
                Ok(())
            }
        }
    };
    ...
    (tuplestruct $target:ident $($el:tt)*) => {
        impl<'de> $crate::BinDeserialize<'de> for $target {
            fn deserialize<D: $crate::BinDeserializer<'de>>(mut deserializer: D) -> $crate::Result<Self> {
                $(let $crate::member_to_ident!($el) = $crate::BinDeserialize::deserialize(&mut deserializer)?;)*
